...
nnoremap <space>w :set wrap<cr>:setlocal spell spelllang=en_us<cr>:colorscheme gruvbox<cr>:set background=light<cr>:set syntax=markdown<cr>:echo "  ]s next word\n  [s previous word\n  zG ignore the word\n  zg add word to dictionary"<cr>
nnoremap <space>W :set number<cr>:set nowrap<cr>:set nospell<cr>:colorscheme gruvbox<cr>:set background=dark<cr>:filetype detect<cr>
